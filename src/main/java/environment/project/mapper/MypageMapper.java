...
public interface MypageMapper {
    ...
    // 회원 이메일, 닉네임, 전화번호 수정
    @Update("UPDATE user SET user_tel = #{ userTel },user_nickName = #{ userNickname }, user_email = #{ userEmail } WHERE user_id = #{ userId }  ")
    void updateUserInfo(UserDTO userDTO, @Param("userId") String userId);
...
//    List<HashMap<String, Object>> selectUserClub(@Param("userId") String userId);
    ...
    // 동아리신청 회원 정보 불러오기
    @Select("SELECT user_name, club_join_date,user_email,user_nickname,user_tel FROM club_info JOIN user ON club_info.user_no = user.user_no WHERE user_id = #{ userId } ")
    List<HashMap<String,Object>> selectClubApplicationMemInfo(@Param("userId") String userId);
}
